...
CAPEInfo::CAPEInfo(int * pErrorCode, const wchar_t * pFilename, CAPETag * pTag) 
{
    *pErrorCode = ERROR_SUCCESS;
    ...
    char *pAnsiFileName = CAPECharacterHelper::GetANSIFromUTF16(pFilename);
    if (m_spIO->Open(pAnsiFileName) != 0)
    {
        delete [] pAnsiFileName;
        CloseFile();
...
CAPEInfo::CAPEInfo(int * pErrorCode, CStdLibFileIO * pIO, CAPETag * pTag)
{
    *pErrorCode = ERROR_SUCCESS;
    CloseFile();
...
int CAPEInfo::CheckHeaderInformation()
{
    ...
    {
        int nFileBytes = int(m_spIO->GetSize());
        if (nFileBytes > 0)
        {
...
*****************************************************************************************/
unsigned long long CAPEInfo::GetInfo(APE_DECOMPRESS_FIELDS Field, unsigned long long nParam1, unsigned long long nParam2)
{
    ...
    switch (Field)
    {
        ...
        {
            unsigned long long nFrame = nParam1;
            
            nRetVal = 0;
            
            unsigned long long nFrameBytes = GetInfo(APE_INFO_FRAME_BYTES, nFrame);
            unsigned long long nFrameBlocks = GetInfo(APE_INFO_FRAME_BLOCKS, nFrame);
            if ((nFrameBytes > 0) && (nFrameBlocks > 0) && m_APEFileInfo.nSampleRate > 0)
            {
                unsigned long long nFrameMS = (nFrameBlocks * 1000) / m_APEFileInfo.nSampleRate;
                if (nFrameMS != 0)
                {
        ...
        case APE_INFO_SEEK_BIT:
        {
            ...
            {
                if (nFrame >= m_APEFileInfo.nTotalFrames)
                    nRetVal = 0;
                else
        ...
        case APE_INFO_WAV_HEADER_DATA:
        {
            ...
            unsigned long long nMaxBytes = nParam2;
        ...
        case APE_INFO_WAV_TERMINATING_DATA:
        {
            ...
            else
            {
                ...
                {
                    // variables
                    unsigned long long nOriginalFileLocation = m_spIO->GetPosition();
        ...
        case APE_INFO_IO_SOURCE:
            nRetVal = (unsigned long long) m_spIO.GetPtr();
            break;
        ...
        case APE_INFO_TAG:
            nRetVal = (unsigned long long) m_spAPETag.GetPtr();
            break;
        case APE_INTERNAL_INFO:
            nRetVal = (unsigned long long) &m_APEFileInfo;
            break;
        default:
            break;
    }
