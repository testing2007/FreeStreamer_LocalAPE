...
CAPETag::CAPETag(const str_utf16 * pFilename, BOOL bAnalyze)
{
    ...
    CSmartPtr<char> ptrAnsiFileName(CAPECharacterHelper::GetANSIFromUTF16(pFilename), TRUE);
    m_spIO->Open(ptrAnsiFileName.GetPtr());
...
CAPETag::CAPETag(CStdLibFileIO * pIO, BOOL bAnalyze)
{
...
{
    int nOriginalPosition = (int)m_spIO->GetPosition();
...
int CAPETag::CreateID3Tag(ID3_TAG * pID3Tag)
{
    ...
    char *pAnsi = CAPECharacterHelper::GetANSIFromUTF16(cBuffer);
    pID3Tag->Track = (unsigned char) atol(pAnsi);
