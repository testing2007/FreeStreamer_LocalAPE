...
int CStdLibFileIO::Seek(long long nDistance, unsigned int nMoveMode)
{
...
long long CStdLibFileIO::GetPosition()
{
    fpos_t fPosition;
...
long long CStdLibFileIO::GetSize()
{
    long long nCurrentPosition = GetPosition();
    Seek(0, FILE_END);
    long long nLength = GetPosition();
...
int CStdLibFileIO::Create(const wchar_t * pName)
{
    ...
    if (0 == _wcsicmp (pName, L"-") || 0 == _wcsicmp (pName, L"/dev/stdout"))
    {
        m_pFile = SETBINARY_OUT(stdout);
        m_bReadOnly = FALSE;                            // WriteOnly
    }
    ...
    {
        char *pAnsiName = APE_MONKEY::CAPECharacterHelper::GetANSIFromUTF16(pName);
        m_pFile = fopen (pAnsiName, "wb");                  // Read/Write
        delete [] pAnsiName;
        m_bReadOnly = FALSE;
    }
