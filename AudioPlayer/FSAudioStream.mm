...
- (id)init
{
    ...
    if (self) {
        ...
        self.bufferCount    = 8;
        self.bufferSize     = 32768; //32*1024
//        self.bufferSize     = 65536; //2*32*1024
        self.maxPacketDescs = 512; //2^9
        self.decodeQueueSize = 128; //2^7
        self.httpConnectionBufferSize = 1024; //2^10
...
//        self.maxDiskCacheSize = 256000000; // 256 MB //改用调用kwconfig设置
        self.maxDiskCacheSize = g_config->getCachesValue()*1000*1000; //用kwconfig设置
//        if(self.maxDiskCacheSize<256000000)
//        {
//            NSLog(@"cache size is less than 256M");
//        }
...
//        }
        self.cacheDirectory = g_config->cacheDirectory;
...
{
    _reachability = [Reachability reachabilityWithHostname:@"www.kuwo.cn"];
    
    __weak typeof(self) wSelf = self;
    _reachability.reachableBlock = ^(Reachability *reach) {
        if (wSelf.wasDisconnected) {
            wSelf.wasDisconnected = NO;
            
#if defined(DEBUG) || (TARGET_IPHONE_SIMULATOR)
            NSLog(@"FSAudioStream: Internet connection available again.");
#endif
            [wSelf attemptRestart];
        }
    };
    ...
    _reachability.unreachableBlock = ^(Reachability *reach) {
        if ([wSelf isPlaying])
        {
            wSelf.wasDisconnected = YES;
            wSelf.onFailure(kFsAudioStreamErrorNetwork, @"FSAudioStream: Error: Internet connection disconnected while playing a stream.");
...
#endif
        self.onFailure(kFsAudioStreamErrorNetwork, @"FSAudioStream: Error: Internet connection disconnected while playing a stream.");
