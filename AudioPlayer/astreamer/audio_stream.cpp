...
Audio_Stream::Audio_Stream() :
    ...
    m_localUnsupportCodecRunning(false),
...
{
    if (m_inputStreamRunning || m_audioStreamParserRunning || m_localUnsupportCodecRunning) {
...
void Audio_Stream::close(bool closeParser)
{
    ...
    if(closeParser && m_localUnsupportCodecRunning) {
        m_localUnsupportCodecRunning = false;
    }
...
{
    if(m_localUnsupportCodecRunning)
    {
...
AS_Playback_Position Audio_Stream::playbackPosition()
{
    ...
    else if(m_localUnsupportCodecRunning)
    {
        AudioTimeStamp queueTime = audioQueue()->currentTime();
...
float Audio_Stream::durationInSeconds()
{
    if(m_localUnsupportCodecRunning)
    {
        return ((APEFile_Stream*)m_inputStream)->durationInSeconds();
    }
...
//        }
    ...
    {
        AS_TRACE("%s: enter, localUnsupportCodecRunning", __PRETTY_FUNCTION__);
    }
    ...
    UInt64 originalContentLength = contentLength();// m_contentLength;
...
void Audio_Stream::setUrl(CFURLRef url)
{
    ...
    } else if(APEFile_Stream::canHandleUrl(url)){
        m_inputStream = new APEFile_Stream();
        m_inputStream->m_delegate = this;
//        m_unsupportCodec = true;
    }
    else if (File_Stream::canHandleUrl(url)) {
        m_inputStream = new File_Stream();
        m_inputStream->m_delegate = this;
//        m_unsupportCodec = false;
    }
...
void Audio_Stream::audioQueueStateChanged(Audio_Queue::State state)
{
    ...
    } else if (state == Audio_Queue::IDLE || state==Audio_Queue::EXIT) {
        setState(STOPPED);
...
void Audio_Stream::streamIsReadyRead(bool bUnsupportCodec/*=false*/, AudioStreamBasicDescription* dstFormat/*=NULL*/)
{
    if (m_audioStreamParserRunning || m_localUnsupportCodecRunning) {
    ...
    else
    {
        if(dstFormat!=NULL)
        {
            m_localUnsupportCodecRunning = true;
            m_dstFormat = *dstFormat;
            audioQueue()->init();
        }
        else
        {
            closeAndSignalError(AS_ERR_OPEN, CFSTR("Audio file stream format error"));
        }
    }
}
...
//        The number of bytes in the packet.
        ...
        packetDesc.mDataByteSize = numBytes;
        packetDesc.mStartOffset = 0;
        packetDesc.mVariableFramesInPacket = 0;
        audioQueue()->handleAudioPackets(numBytes, 1, data, &packetDesc);
    }
}
...
void Audio_Stream::streamEndEncountered()
{
    ...
    if (!(contentLength() > 0)) {
        ...
        if(m_localUnsupportCodecRunning)
        {
            closeAndSignalError(AS_ERR_OPEN, CFSTR("Stream ended abruptly"));
        }
        else
        {
...
void Audio_Stream::streamErrorOccurred(CFStringRef errorDesc)
{
    ...
    if(m_localUnsupportCodecRunning)
    {
        closeAndSignalError(AS_ERR_OPEN, errorDesc);
    }
    else
    {
...
const    char* Audio_Stream::getStateDesc(State state)
    {
        switch (state) {
            case STOPPED:
                return "STOPPED";
...
void Audio_Stream::setState(State state)
{
    ...
    AS_TRACE("%s: curState=%s, newState=%s\n", __PRETTY_FUNCTION__, getStateDesc(m_state), getStateDesc(state));
    m_state = state;
...
//    return ;//####
    if(m_localUnsupportCodecRunning)
    {
        AS_TRACE("ignore watchdog timer for the unsupport codec file");
        return ;
    }
