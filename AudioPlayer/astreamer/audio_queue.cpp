...
AudioTimeStamp Audio_Queue::currentTime()
{
    ...
    if (err) {
        AQ_TRACE("AudioQueueGetCurrentTime failed, the error code=%ld\n", err);
//        setState(EXIT);
    }
...
int Audio_Queue::handlePacket(const void *data, AudioStreamPacketDescription *desc)
{
    ...
    if ((int32_t)(config->bufferSize - m_bytesFilled) < (int32_t)packetSize) {
...
int Audio_Queue::enqueueBuffer()
{
    ...
    AQ_TRACE("%s: enter\n", __PRETTY_FUNCTION__);
//    AQ_TRACE("%s: m_fillBufferIndex=%d\n", __PRETTY_FUNCTION__, m_fillBufferIndex);
    ...
    if (!err) {
        ...
        m_lastError = err;
        m_bufferInUse[m_fillBufferIndex] = false;
        m_buffersUsed--;
        return -1;
    }
